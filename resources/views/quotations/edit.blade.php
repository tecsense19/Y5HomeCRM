        ...
        <form method="POST" action="{{ route('quotations.update', $quotation) }}" enctype="multipart/form-data">
            ...
            <div class="card mb-3">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('quotations.show', $quotation) }}" class="btn btn-outline-secondary btn-sm">Cancel</a>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Link to Opportunity</label>
                            <select class="form-select" disabled>
                                <option value="">— None —</option>
                                @if($quotation->opportunity)
                                    <option value="{{ $quotation->opportunity_id }}" selected>
                                        {{ $quotation->opportunity->opportunity_number }} – {{ $quotation->opportunity->customer_name }}
                                    </option>
                                @endif
                            </select>
                            <small class="text-muted">Associated opportunity cannot be changed.</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="customer_name">Customer Name *</label>
                            <input type="text" name="customer_name" id="customer_name" class="form-control" value="{{ old('customer_name', $quotation->customer_name) }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label" for="quotation_date">Quotation Date *</label>
                            <input type="date" name="quotation_date" id="quotation_date" class="form-control" value="{{ old('quotation_date', $quotation->quotation_date ? $quotation->quotation_date->format('Y-m-d') : '') }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label" for="quotation_value">Quotation Value (₹)</label>
                            <input type="number" step="0.01" name="quotation_value" id="quotation_value" class="form-control" value="{{ old('quotation_value', $quotation->quotation_value) }}">
                        </div>
                        <div class="col-md-4">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    {{-- Existing documents --}}
                    @if($quotation->documents && $quotation->documents->count() > 0)
                        <div class="mb-3">
                            <h6 class="fw-semibold mb-2"><i class="bi bi-paperclip me-1"></i> Attached Documents</h6>
                                ...
                                @foreach($quotation->documents as $doc)
                                    <div class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <div>
                                                <div class="fw-semibold small">{{ $doc->original_name ?? basename($doc->file_path) }}</div>
                                                <div class="text-muted" style="font-size:0.75rem;">
                                                    {{ $doc->category ? ucfirst(str_replace('_', ' ', $doc->category)) : 'Document' }}
                                        ...
                                        <div class="d-flex gap-2 align-items-center">
                                            ...
                                            <form method="POST" action="{{ route('quotations.documents.destroy', [$quotation, $doc]) }}"
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label class="form-label" for="drawings">CAD Drawings / Layout Files</label>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary px-4">Update Quotation</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Items logic
        let items = [];
        const oldItemsRaw = @json(old('items', $quotation->items ?? []));
        if (oldItemsRaw) {
